...
void Player::load(std::unique_ptr<LoaderParams> const &pParams)
{
  ...
  // Death explosion time
  m_dyingTime = 100;

  TheCamera::getInstance()->setTarget(&m_position);
}
...
{
  TheGame::getInstance()->setPlayerLives(TheGame::getInstance()->getPlayerLives() - 1);
...
{
  if (TheInputHandler::getInstance()->isKeyDown(SDL_SCANCODE_RIGHT) && m_position.m_x < ((*m_pCollisionLayers->begin())->getMapWidth() * 32))
  {
  ...
  else if (TheInputHandler::getInstance()->isKeyDown(SDL_SCANCODE_LEFT) && m_position.m_x > 32)
  {
    if (TheInputHandler::getInstance()->isKeyDown(SDL_SCANCODE_A))
    {
      m_bRunning = true;
  ...
  if (!TheInputHandler::getInstance()->isKeyDown(SDL_SCANCODE_SPACE) && m_bCanJump)
  {
    m_bPressedJump = false;
  }
}